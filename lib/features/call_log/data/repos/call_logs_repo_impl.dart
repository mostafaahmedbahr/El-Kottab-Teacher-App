  ...
  Future<Either<Failure, CallsModel>> getAllCalls(String? type) async {
    try {
      var response = await apiService!.getData(
        ...
        query: type == null ? null : {"status": type},
      );
  ...
  Future<Either<Failure, CallRevisionModel>> addCallRevision(
    ...
    try {
      var response = await apiService!.postData(
        endPoint: 'call-revisions',
        data: {'call_id': callId, 'revision': revision},
      );

      CallRevisionModel result = CallRevisionModel.fromJson(response.data);
      return right(result);
    } catch (e) {
      return left(handleError(e));
    }
