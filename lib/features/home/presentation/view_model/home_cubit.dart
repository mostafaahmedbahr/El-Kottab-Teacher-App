...
class HomeCubit extends Cubit<HomeStates> {
  ...
  void changeStatus(bool newStatus) {
    ...
    updateAvailability(status: newStatus ? "active" : "inactive");
  }
  ...
  Future<void> getTeacherStats({required int teacherId}) async {
  ...
  Future<void> updateAvailability({required String status}) async {
  ...
  TeacherPerformanceModel? teacherPerformanceModel;

  Future<void> getTeacherPerformance({required int teacherId}) async {
    emit(GetTeacherPerformanceLoadingState());

    var result = await homeRepo!.getTeacherPerformance(teacherId: teacherId);

    result.fold(
      (failure) {
        emit(GetTeacherPerformanceErrorState(failure.errMessage));
      },
      (performanceModel) {
        teacherPerformanceModel = performanceModel;

        emit(GetTeacherPerformanceSuccessState(teacherPerformanceModel!));
      },
    );
  }
}
